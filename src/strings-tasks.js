...
function findLongestWord(sentence) {
  ...
  words.forEach((word, index) => {
    if (word.length > words[findIndex].length) {
      findIndex = index;
    }
  });
...
function encodeToRot13(str) {
  return str.replace(/[a-z]/gi, (char) => {
    ...
    const num = code >= 65 && code <= 90 ? 65 : 97;
    return String.fromCharCode(((code - num + 13) % 26) + num);
  });
}
...
function getCardId(value) {
  ...
  const index = cards.indexOf(mast);
  const strongIndex = val.indexOf(item);
  return index * val.length + strongIndex;
}
